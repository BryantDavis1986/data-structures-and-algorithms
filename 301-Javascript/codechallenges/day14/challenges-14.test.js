...
const toTitleCase = (arr) => {
  ...
  const upperCase = arr.map(value =>{
      const restWord = `${value.charAt(0).toUpperCase()}${value.slice(1)}`
    return restWord;
  })
 return upperCase;
};
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  let str = ``;
  const largerMass = arr.forEach(value => {
      let skywalker = parseInt(value.mass);
      let notSkywalker = parseInt(arr[0].mass);
      if(skywalker > notSkywalker){
          str = str + value.name + ' - ';
      }
  })
  let newStr = str.substring(0, 23);
  return newStr;
};
