...
const templateWithJQuery = () => {
  ...
  starWarsPeople.forEach((value) => {
    ...
    template2.attr('id', 'blah');
    template2.find('h2').text(value.name);
    template2.find('h3').text(value.height);
    template2.find('p').text(value.eye_color);
    $('main').append(template2);
...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(value => {
      let stuff = value.slice(value.indexOf(' ')+1);
      let slice2 = stuff.slice(stuff.indexOf(' ')+1);
      result.push(slice2);
  })
