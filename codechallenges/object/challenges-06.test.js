...
const templatingWithMustache = () => {
  ...
  characters.forEach(value => {
      let template = $('h2').html();
      let rendered = Mustache.render(template, {name: value.name})
      array.push(rendered)
      let template2 = $('h3').html();
      let rendered2 = Mustache.render(template2, {spouse: value.spouse})
      array.push(rendered2)
...
const hasChildrenValues = (arr, character) => {
 ...
 Object.values(arr.forEach(value => {
     console.log(value.name.includes(character) && value.children.length > 0)
      if(value.name.includes(character) && value.children.length > 0){
          return true;
        //   break;
      }else {
          return false;
