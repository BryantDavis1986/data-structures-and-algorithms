...
route that sends a short bio about you to the front-end. Finally, connect a '/favoritefoods' route that sends an array to the front-end of 
your favorite foods. All other routes should respond with a status of 404.
------------------------------------------------------------------------------------------------ */
...
app.get('/aboutme', (request, response) => {
    const bio = 'My name is Bryant, I am 34 years old etc'
    response.send(bio);
});
app.get('/favoritefoods', (request, response) => {
    const favFoods = ['pizza', 'spaghetti', 'burritos'];
...
const count = (target, input) => {
  // Solution code here...
  let sum = 0;
  const totalCounted = input.map(value => {
     const counted = value.reduce((num, valu) => {
          if(valu === target){
              return num +1;
          }else {
              return num +0;
